...
@app.route('/generate_csrf_token', methods=['POST'])
#@csrf.exempt
...
def create_company():
    ...
    try:
        ...
        gas_estimate = contract.functions.createCompany(company_id).estimate_gas({'from': account.address})
        ...
        if 'execution reverted' in error_message:
            if 'Company already exists' in error_message:
                return jsonify({'error': 'Company already exists'}), 400
        
        return jsonify({'error': 'An error occurred while creating the company' + str(e)}), 500
...
def add_data():
    ...
    try:
        ...
        # Calcular el precio del gas dinámicamente (ejemplo simplificado)
        gas_price = w3.eth.generateGasPrice()  # Obtiene el precio promedio actual del gas
